...
u=1 #this is relaxation on sign prediction
if len(sys.argv)>1:
...
secondvectors_test= [{} for _ in range(totalvectors)] #this is final generated vector out of the present ith tree
...
for treecounter in range(100):
    ...
    for examplei in range(0,totalvectors):
        ...
        if decidedlabel==1:
            secondvectors_test[examplei][treecounter]=decidedlabel
        else:
            secondvectors_test[examplei][treecounter]=0
    # root.printpreorder()
...
if maxlevel==4:
	file_name='badges-ensemble-features-test_4.txt'
elif maxlevel==8:
	file_name='badges-ensemble-features-test_8.txt'	
elif maxlevel==20:
	file_name='badges-ensemble-features-test_20.txt'	
		
f = open(file_name, 'w' )


for i in range(0, len(secondvectors_test)):
		...
		for j in range(0, 100):
			val2=str(secondvectors_test[i][j])
			if (j!=99):
